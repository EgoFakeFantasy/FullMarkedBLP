...
/-- A block's row index is its position plus the fixed original owner. -/
def BlockCoreValid (base : Nat) (block : Pattern) : Prop :=
  ∀ i (hi : i < block.length), (block[i]).CoreValid (base + i)

theorem nativeBlockDown_short_total (k : Nat) {base : Nat} {top : Row}
    (hv : top.CoreValid (base + k))
    (hlen : top.core.length + 1 = 2 * top.step)
    (hstep : k + 3 ≤ top.step)
    (htarget : ∀ x, base ≤ x → x ≤ base + k → x ∈ top.core) :
    ∃ block, nativeBlockDown k (base + k) false top = some block ∧ BlockCoreValid base block := by
  ...
  | succ k ih =>
    ...
    obtain ⟨earlier, he, hev⟩ := ih hvalid hshape (by omega)
...
theorem nativeBlockDown_medium_total (k : Nat) {base : Nat} {top : Row}
  ...
  obtain ⟨earlier, he, hev⟩ := nativeBlockDown_short_total k hvalid hshape.2 hstep ht
  have heLen := nativeBlockDown_length he
  refine ⟨earlier ++ [top], ?_, ?_⟩
  · simp [nativeBlockDown, hl, hprev, he]
  · intro i hi
