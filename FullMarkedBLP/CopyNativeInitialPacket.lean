...
copy creates a complete initial segment of that predecessor block. The segment
width is the actual output length, bounded by the predecessor record length. -/
theorem shortCopy_native_birth_initial_packet {lambda : Ordinal.{u}} {parent copied a b : Pattern}
    ...
    sources.length ≤ lowerSources.length ∧ ∀ k, 0 < k → k ≤ sources.length →
  ...
  | cons head tail =>
    ...
    have target : base + (head - base) = head := by omega
    have first : (rowAt (completeFrozenMarks a rec r) e).bind Row.b = some (base + (head - base)) := by
      simpa only [target, ← eq] using firstB
    obtain ⟨length, packet⟩ := native_birth_exact_packet_of_first_target reach currentReal historyValid
      hr hp he record events birth nonempty (by omega : 0 < head - base) (by omega : head - base ≤ lowerSources.length) first
    refine ⟨by omega, ?_⟩
    intro k hk hkt
    exact packet k hk (by omega)
