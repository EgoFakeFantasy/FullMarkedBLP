...
theorem shortCopy_scan_owner_endpoint_boundary {lambda : Ordinal.{u}} {parent copied a : Pattern}
    ...
    ∃ w, (rowAt a e).bind Row.b = some w ∧ (w ≤ base ∨ w = base + lowerSources.length) := by
  obtain ⟨phi, original, mono, zero, lengths, holds, tail, rows, records, boundaries⟩ :=
    shortCopy_scan_b_boundaries parentValid sat copy reach entryReal
  have located := boundaries verified
  ...
  rcases located oldE er v atEr atV with unchanged | ⟨ss, headRecord, topValue⟩
  · refine ⟨phi v, by simpa only [endpointMap] using unchanged, Or.inl ?_⟩
    have bound := mono.monotone vBound
    simpa only [lowerMap] using bound
  · refine ⟨phi v + ss.length, by simpa only [endpointMap] using topValue, ?_⟩
    by_cases lt : v < lower
    · obtain ⟨i, _, image, successor⟩ := records (phi v) ss headRecord
      have same : i = v := mono.injective image
      subst i
      have bound : phi (v + 1) ≤ phi lower := mono.monotone (by omega)
      rw [successor, lowerMap] at bound
      exact Or.inl (by omega)
    · have same : v = lower := by omega
      subst v
      have atBase : (base, ss) ∈ rec := by simpa only [lowerMap] using headRecord
      have eq := scanReach_record_unique (scanEmbeddingReach_forget (scanRankReach_embeddings reach)) atBase record
      exact Or.inr (by rw [lowerMap, eq])
