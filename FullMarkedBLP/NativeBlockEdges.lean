...
def BlockEdges {alpha : Type u} (action : alpha → alpha) (theta : Nat → alpha)
    (base : Nat) (block : Pattern) : Prop :=
  ∀ i (hi : i < block.length), (block[i]).RealizesEdges action theta (base + i)

theorem nativeBlockDown_short_edges (k : Nat) {alpha : Type u} (action : alpha → alpha) (theta : Nat → alpha) {base : Nat} {top : Row}
    (hv : top.CoreValid (base + k)) (hmarks : top.ProperMarks (base + k))
    (htraces : top.RealizesEdges action theta (base + k))
    (hlen : top.core.length + 1 = 2 * top.step) (hstep : k + 3 ≤ top.step)
    (htarget : ∀ x, base ≤ x → x ≤ base + k → x ∈ top.core) :
    ∃ block, nativeBlockDown k (base + k) false top = some block ∧ BlockEdges action theta base block := by
  ...
  | succ k ih =>
    ...
    have htraceLower := nativeLower_short_realizes_edges action theta hv (by omega) hlen (by omega) hl htraces
    rw [hprev] at hvalid hproper htraceLower
...
theorem nativeBlockDown_medium_edges (k : Nat) {alpha : Type u} (action : alpha → alpha) (theta : Nat → alpha) {base : Nat} {top : Row}
    (hv : top.CoreValid (base + (k + 1))) (hmarks : top.ProperMarks (base + (k + 1)))
    (htraces : top.RealizesEdges action theta (base + (k + 1)))
    ...
    ∃ block, nativeBlockDown (k + 1) (base + (k + 1)) true top = some block ∧
      BlockEdges action theta base block := by
  ...
  obtain ⟨earlier, he, hev⟩ := nativeBlockDown_short_edges k action theta hvalid hproper (by simpa only [hprev] using nativeLower_medium_realizes_edges action theta hv (by omega) hl htraces) hshape.2 hstep ht
  have heLen := nativeBlockDown_length he
  refine ⟨earlier ++ [top], ?_, ?_⟩
  · simp [nativeBlockDown, hl, hprev, he]
  · intro i hi
