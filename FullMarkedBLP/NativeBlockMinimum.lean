...
def BlockMinimum (minimum : Nat) (block : Pattern) : Prop :=
  ∀ i (hi : i < block.length), (block[i]).core.head? = some minimum

theorem nativeBlockDown_short_minimum (k : Nat) {minimum : Nat} {base : Nat} {top : Row}
    (hv : top.CoreValid (base + k)) (hmarks : top.ProperMarks (base + k))
    (htraces : top.core.head? = some minimum)
    (hlen : top.core.length + 1 = 2 * top.step) (hstep : k + 3 ≤ top.step)
    (htarget : ∀ x, base ≤ x → x ≤ base + k → x ∈ top.core) :
    ∃ block, nativeBlockDown k (base + k) false top = some block ∧ BlockMinimum minimum block := by
  ...
  | succ k ih =>
    ...
    have htraceLower := nativeLower_preserves_minimum hv htraces hl
...
theorem nativeBlockDown_medium_minimum (k : Nat) {minimum : Nat} {base : Nat} {top : Row}
  ...
  obtain ⟨earlier, he, hev⟩ := nativeBlockDown_short_minimum k hvalid hproper (nativeLower_preserves_minimum hv htraces hl) hshape.2 hstep ht
  have heLen := nativeBlockDown_length he
  refine ⟨earlier ++ [top], ?_, ?_⟩
  · simp [nativeBlockDown, hl, hprev, he]
  · intro i hi
