...
def BlockRankCertificates {lambda : Ordinal.{u}} (a : Pattern)
    (theta : Nat → OrdinalDomain lambda) (embedding : Nat → RankElementaryEmbedding lambda)
    (ownerEmbedding : RankElementaryEmbedding lambda) (block : Pattern) : Prop :=
  ∀ i (hi : i < block.length), (block[i]).HasRankCertificates a theta embedding ownerEmbedding

theorem nativeBlockDown_short_rankCertificates (k : Nat) {lambda : Ordinal.{u}} {a : Pattern}
    {theta : Nat → OrdinalDomain lambda} {embedding : Nat → RankElementaryEmbedding lambda}
    {ownerEmbedding : RankElementaryEmbedding lambda} {base : Nat} {top : Row}
    (hv : top.CoreValid (base + k)) (hmarks : top.ProperMarks (base + k))
    (htraces : top.HasRankCertificates a theta embedding ownerEmbedding)
    (hlen : top.core.length + 1 = 2 * top.step) (hstep : k + 3 ≤ top.step)
    (htarget : ∀ x, base ≤ x → x ≤ base + k → x ∈ top.core) :
    ∃ block, nativeBlockDown k (base + k) false top = some block ∧ BlockRankCertificates a theta embedding ownerEmbedding block := by
  ...
  | succ k ih =>
    ...
    have htraceLower := nativeLower_short_rankCertificates hv hmarks hlen htraces hl
...
theorem nativeBlockDown_medium_rankCertificates (k : Nat) {lambda : Ordinal.{u}} {a : Pattern}
    {theta : Nat → OrdinalDomain lambda} {embedding : Nat → RankElementaryEmbedding lambda}
    {ownerEmbedding : RankElementaryEmbedding lambda} {base : Nat} {top : Row}
    ...
    ∃ block, nativeBlockDown (k + 1) (base + (k + 1)) true top = some block ∧
      BlockRankCertificates a theta embedding ownerEmbedding block := by
  ...
  obtain ⟨earlier, he, hev⟩ := nativeBlockDown_short_rankCertificates k hvalid hproper (nativeLower_medium_rankCertificates hv hmarks htraces hl) hshape.2 hstep ht
  have heLen := nativeBlockDown_length he
  refine ⟨earlier ++ [top], ?_, ?_⟩
  · simp [nativeBlockDown, hl, hprev, he]
  · intro i hi
