...
theorem rankCoherentRoots_of_sigmaTwo {lambda : Ordinal.{u}} (hl : Order.IsSuccLimit lambda)
  ...
  exact rankCoherentRoots_of_sigmaTwo_and_lowRank_definability hl cp elementary
...
theorem rankI2_full_root (i2 : RankI2.{u}) :
  ...
  obtain ⟨roots, _⟩ := rankCoherentRoots_of_sigmaTwo hl cp elementary
...
domain. This proves expansion well-foundedness; the final comparison
well-order and short-key injectivity are separate remaining obligations. -/
