...
/-- Adjacent retained factors inherit their whole actual upper-width packet
from the original Sat short-copy birth, without a source-entry hypothesis. -/
theorem shortCopy_recorded_initial_packet {lambda : Ordinal.{u}} {parent copied a : Pattern}
  ...
  obtain ⟨width, packet⟩ := shortCopy_native_birth_initial_packet parentValid sat copy labelled entryReal h prior
    hr oldP atE lowerAtBirth (events row hr) birth nonempty
  refine ⟨width, ?_⟩
  intro k hk hkt
  obtain ⟨member, edge⟩ := packet k hk hkt
