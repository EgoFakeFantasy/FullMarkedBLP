...
theorem scanRankReach_frozen_historical_certificates {lambda : Ordinal.{u}} {initial a : Pattern}
  ...
  intro y hy
  obtain ⟨xs, delta, trace, _, cutoff, saved⟩ := certs y hy
  have transported := frozen_fold_preserves_historical_trace processed events trace
  exact ⟨xs, delta, transported, computeMarkTrace_complete valid transported, cutoff, saved⟩
